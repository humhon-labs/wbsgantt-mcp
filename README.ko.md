...
- "이 프로젝트 일정이 건강한지 점검해줘" — 일정 건강도(FS 비율·lag·고정일)

## 🚀 3단계 연결 (설치 불필요)

1. **토큰 발급** — wbsgantt 로그인 → 우상단 프로필 메뉴 → **계정 설정 → API 토큰** 에서 발급 (권한: *읽기 전용* 또는 *읽기+쓰기*). 토큰은 **발급 직후 1회만** 표시되니 복사해 두세요.
2. **커넥터 등록** — 아래 MCP 서버 주소를 LLM 클라이언트에 등록하고, 토큰은 **Bearer 토큰**으로 전달합니다(`Authorization: Bearer <토큰>` 헤더 또는 클라이언트의 토큰 입력란).
...
- 토큰은 `bwic_pat_` 로 시작하며 발급일로부터 **90일 후 자동 만료**, 사용자당 최대 10개. 유출 의심 시 계정 화면에서 즉시 **폐기**하세요.
- 구조 규칙(마일스톤 기간 0·순환 종속성 차단)은 서버에서 검증되어 잘못된 편집이 오류로 거부되고 LLM이 스스로 바로잡습니다. 100% Rule 같은 품질 규칙은 `get_schedule_health` 로 진단해 LLM이 검토·수정할 수 있습니다.
